...
//                  error_log("Relative Resource folder : ".$resFolder);
                    return $resFolder;
			}
            ...
            */
            ...
            public function updateColumn($column,$value,$table)
            {
                $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable($table);
                $affectedRows = $queryBuilder->insert($table)->values([ $column => $value, ])->execute();
            }

            //Fetch a value from Databse
            public function fetchValueFromDb($col, $table){
            if (!$this->isEmptyOrNull($col) && !$this->isEmptyOrNull($table)){
                $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable($table);
                ...
                error_log("Error invalid column or table name");
            }
